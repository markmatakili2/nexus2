...
import { User, Student, Term, SchoolClass, Mark, ExamSession, Subject, UserRole, School } from '../types';
...
export const DashboardPage: React.FC<DashboardPageProps> = ({
    currentUser, users, students, currentTermId, terms, classes, activeSubjects, marks, examSessions, subjects, currentSchool
}) => {
    ...
    const parentChild = useMemo(() => currentUser.roles.includes(UserRole.PARENT) && currentUser.studentId ? students.find(s => s.id === currentUser.studentId) : null, [currentUser, students]);
    ...
    const parentPerformanceHistory = useMemo(() => {
        if (parentChild && activeTerm) {
            return getStudentPerformanceHistory(parentChild.id, activeTerm, students, terms, classes, activeSubjects, marks, examSessions, users, subjects);
        }
        return [];
    }, [parentChild, activeTerm, students, terms, classes, activeSubjects, marks, examSessions, users, subjects]);
    ...
    return (
      <PageWrapper title={`Dashboard - ${activeTerm?.name || ''} ${activeTerm?.year || ''}`}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow col-span-1 md:col-span-1 lg:col-span-1">
            <div className="flex items-center mb-3">
              ...
              <div>
                ...
                <p className="text-gray-600 dark:text-slate-300 text-sm">Role: {currentUser.roles.join(', ')}</p>
              </div>
            </div>
          ...
          {(currentUser.roles.includes(UserRole.ADMIN) || currentUser.roles.includes(UserRole.TEACHER)) && (
          ...
          {currentUser.roles.includes(UserRole.PARENT) && parentChild && activeTerm && (
             <div className="col-span-1 md:col-span-2 lg:col-span-3 mt-6 relative"> 
                ...
                <ReportDisplay report={parentReport} performanceHistory={parentPerformanceHistory} actualSchoolName={currentSchool?.name || ''} schoolAddress={currentSchool?.address || ''} userRoles={currentUser.roles} schoolBadgeUrl={currentSchool?.badgeUrl} schoolMotto={currentSchool?.motto} />
            </div>
          )}
        </div>

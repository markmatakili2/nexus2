...
import { Student, Term, SchoolClass, Subject, Mark, ExamSession, PerformanceChangeEntry, User, UserRole, StudentPerformanceDatapoint, School } from '../types';
import { generateMeritList, generatePerformanceChanges, getStudentPerformanceHistory } from '../utils/examLogic';
...
export const MeritListPage = ({
  ...
  selectedComparisonTermId, onComparisonTermChange, subjects, currentUser, schoolBadgeUrl, schoolMotto
}: MeritListPageProps): React.ReactElement => {
  ...
  const [performanceHistory, setPerformanceHistory] = useState<StudentPerformanceDatapoint[]>([]);
  ...
  const improvedStudents = useMemo(() => {
    ...
    switch (improvedSortKey) {
        case 'name':
            return filtered.sort((entryA, entryB) => entryA.student.name.localeCompare(entryB.student.name));
        case 'adm':
            return filtered.sort((entryA, entryB) => entryA.student.admissionNumber.localeCompare(entryB.student.admissionNumber, undefined, { numeric: true }));
        case 'change':
        default:
            return filtered.sort((entryA, entryB) => entryB.meanPointsChange - entryA.meanPointsChange);
    }
  ...
  const droppedStudents = useMemo(() => {
    const filtered = performanceChanges.filter(p => p.meanPointsChange < 0);
     switch (droppedSortKey) {
        ...
        case 'change':
        default:
            return filtered.sort((entryA, entryB) => entryA.meanPointsChange - entryB.meanPointsChange);
    }
  }, [performanceChanges, droppedSortKey]);

  const handleShowPerformance = (student: Student) => {
    if (!termToDisplay) return;
    const history = getStudentPerformanceHistory(student.id, termToDisplay, students, terms, classes, activeSubjects, marks, examSessions, users, subjects);
    setPerformanceHistory(history);
    setModalStudent(student);
  };

  const printButton = currentUser?.roles.includes(UserRole.ADMIN) ? (
    <button 
        onClick={handlePrint}
  ...
  return (
    <PageWrapper title={pageTitle} titleControls={meritListData.length > 0 ? printButton : undefined} isPrintable={true}>
      <div className="print-only print-header-container">
          {schoolBadgeUrl && <img src={schoolBadgeUrl} alt={`${actualSchoolName} Badge`} className="print-header-badge" />}
          <div className="print-header-text">{printableTitle}</div>
      </div>
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700 border dark:border-slate-600">
                  ...
                  {meritListData.map(report => (
                      <tr key={report.student.id}>
                          ...
                          <td className="px-4 py-3 text-sm text-gray-800 dark:text-slate-300">
                            <button onClick={() => handleShowPerformance(report.student)} className="hover:underline text-primary-600 dark:text-primary-400 text-left">
                                {report?.student?.name ?? '-'}
                            </button>
                          </td>
      ...
      <footer className="print-only text-center mt-8 pt-4 border-t-2">
          {schoolMotto && <p className="font-semibold italic text-sm text-gray-700">"{schoolMotto}"</p>}
      </footer>
    </PageWrapper>
  );
};

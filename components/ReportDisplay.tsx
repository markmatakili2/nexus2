...
import { AcademicCapIcon, DocumentTextIcon, PrintIcon, UserCircleIcon } from './common/IconComponents';
  ...
  return (
    <div className="mt-6">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 border border-gray-300 text-sm performance-trend-table">
...
export const ReportDisplay: React.FC<ReportDisplayProps> = ({ report, performanceHistory, actualSchoolName, schoolAddress, userRoles, schoolBadgeUrl, schoolMotto }) => {
  ...
  return (
    <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-4xl mx-auto my-8 border-2 border-primary-600 printable-area report-page report-display-container">
      ...
      <header className="mb-8 pb-4 border-b-2 border-primary-200 flex justify-between items-center">
          {/* Left: Student Photo */}
          <div className="flex-shrink-0">
              {student.profileImageUrl ? (
                  <img src={student.profileImageUrl} alt={student.name} className="w-24 h-32 object-cover rounded-md border-2 border-gray-200" />
              ) : (
                  <div className="w-24 h-32 bg-gray-100 rounded-md border-2 border-gray-200 flex items-center justify-center">
          ...
          <div className="text-center">
              <h1 className="text-3xl font-bold text-primary-700">{actualSchoolName}</h1>
              <p className="text-sm text-gray-500 dark:text-slate-400">{schoolAddress}</p>
              <p className="text-xl text-gray-600 mt-2 font-semibold">STUDENT REPORT CARD</p>
          </div>

          {/* Right: School Badge */}
          <div className="flex-shrink-0">
              {schoolBadgeUrl ? (
                  <img src={schoolBadgeUrl} alt="School Badge" className="w-24 h-24 object-contain" />
              ) : (
                  <AcademicCapIcon className="w-24 h-24 text-gray-300" />
              )}
          </div>
      </header>
      ...
      <section className="mb-8 p-4 bg-secondary-50 rounded-lg border border-secondary-200">
        <h4 className="text-md font-semibold text-secondary-800 mb-3">Fees & Important Dates</h4>
      ...
      <footer className="mt-10 pt-4 border-t-2 border-primary-200 text-center text-xs text-gray-500">
        {schoolMotto && <p className="font-semibold italic mb-2 text-gray-600">"{schoolMotto}"</p>}

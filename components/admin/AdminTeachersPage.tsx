...
import { PlusIcon, PencilIcon, TrashIcon, CameraIcon, XMarkIcon, KeyIcon, ShieldCheckIcon } from '../common/IconComponents';
...
const TeacherFormModal: React.FC<{
    ...
    const [formData, setFormData] = useState<TeacherFormData & {profileImageFile?: File | null, signatureImageFile?: File | null, isBoardMember?: boolean}>({ name: '', username: '', roles: [UserRole.TEACHER], password_param: '', teacherId: '', phoneNumber: '', isBoardMember: false });
    ...
    useEffect(() => {
        if (initialTeacherData) {
            setFormData({
                ...
                roles: initialTeacherData.roles,
                phoneNumber: initialTeacherData.phoneNumber || '',
                password_param: '',
                profileImageFile: null,
                signatureImageFile: null,
                isBoardMember: initialTeacherData.isBoardMember || false
            });
            ...
            setFormData({ name: '', username: '', roles: [UserRole.TEACHER], password_param: '', teacherId: '', phoneNumber: '', profileImageFile: null, signatureImageFile: null, isBoardMember: false });
            setPreviewUrl(null);
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        if (name === 'roles') {
             setFormData(prev => ({ ...prev, roles: [value as UserRole] }));
        } else {
    ...
    const handleRemoveSignatureImage = () => {
        setFormData(prev => ({ ...prev, signatureImageFile: null }));
        setSignaturePreviewUrl(null);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!isEditing && !formData.password_param) { alert("Password is required for new users."); return; }
        if (!formData.username.toLowerCase().endsWith(`@${schoolDomain}`)) { alert(`Username must end with @${schoolDomain}`); return; }
        ...
        if (formData.roles?.includes(UserRole.TEACHER)) {
            classSubjectAssignments = Object.entries(assignments).map(([classId, subjectIds]) => ({
                classId,
                subjectIds,
            }));
        } else {
            classSubjectAssignments = [];
            finalFormData.teacherId = undefined; // Clear teacher ID if not a teacher
        }

        if (formData.roles?.includes(UserRole.BOARD_MEMBER)) {
            finalFormData.isBoardMember = true; // Implicitly a board member
        }

        onSave({ ...finalFormData, classSubjectAssignments });
        onClose();
    };

    const isBoardRole = formData.roles?.includes(UserRole.BOARD_MEMBER);
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t dark:border-slate-700">
                        <div>
                         ...
                         <div>
                            ...
                            <input type="text" name="username" value={formData.username} onChange={handleChange} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200" placeholder={`user.name@${schoolDomain}`}/>
                        </div>
                        {formData.roles?.includes(UserRole.TEACHER) && (
                            <div>
                        ...
                        <div>
                         ...
                         <div>
                            ...
                            <select name="roles" value={formData.roles?.[0] || ''} onChange={handleChange} required className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-md dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200">
                                <option value={UserRole.TEACHER}>Teacher</option>
                                <option value={UserRole.ADMIN}>Admin</option>
                                <option value={UserRole.BOARD_MEMBER}>Board Member</option>
                            </select>
                        </div>
                        <div className="flex items-center justify-start pt-2 md:pt-7">
                            <input
                                type="checkbox"
                                id="isBoardMember"
                                name="isBoardMember"
                                checked={isBoardRole || formData.isBoardMember || false}
                                disabled={isBoardRole}
                                onChange={e => setFormData(prev => ({ ...prev, isBoardMember: e.target.checked }))}
                                className="h-4 w-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500 dark:border-slate-500"
                            />
                            <label htmlFor="isBoardMember" className="ml-3 block text-sm font-medium text-gray-700 dark:text-slate-300">
                                School Board Member {isBoardRole && '(Role Requirement)'}
                            </label>
                        </div>
                    </div>

                    {/* Class/Subject Assignments for TEACHER role */}
                    {formData.roles?.includes(UserRole.TEACHER) && (
                        <div className="space-y-3 pt-4 border-t dark:border-slate-700">
                          ...
                          {classes.map(c => (
                            <div key={c.id} className="p-3 border rounded-md bg-gray-50 dark:bg-slate-700/50 dark:border-slate-700">
                              <strong className="font-semibold text-gray-700 dark:text-slate-300">{c.name}</strong>
                                ...
                                {activeSubjects.map(sub => (
                                  <div key={sub.id} className="flex items-center">
                                    <input
                                      ...
                                      checked={(assignments[c.id] || []).includes(sub.id)}
                                      onChange={() => handleAssignmentChange(c.id, sub.id)}
                                    />
                                    <label htmlFor={`${c.id}-${sub.id}`} className="ml-2 text-sm text-gray-600 dark:text-slate-300">{sub.name}</label>
                                  </div>
                                ))}
                              </div>
                            </div>
                    ...
                    <div className="pt-5 flex justify-end space-x-2">
                        ...
                        <button type="submit" className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">Save User</button>
                    </div>
                </form>
            </div>
        </div>
...
export const AdminTeachersPage: React.FC<AdminTeachersPageProps> = ({ users, schoolNameDomain, onTeacherSave, onTeacherDelete, onPasswordReset, classes, activeSubjects }) => {
    ...
    const handleSave = (data: TeacherFormData & {profileImageFile?: File | null, classSubjectAssignments?: {classId: string, subjectIds: string[]}[], signatureImageFile?: File | null}) => {
        onTeacherSave(data);
        handleCloseModal();
    };
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                    <thead className="bg-gray-50 dark:bg-slate-700">
                        <tr>
                            ...
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-300 uppercase tracking-wider">Assignments</th>
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-slate-300 uppercase tracking-wider">Board Member</th>
                    ...
                    <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                        {users.filter(u => !u.roles.includes(UserRole.PARENT) && u.username !== 'superadmin').map(user => (
                            <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/50">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-slate-200">{user.name}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-slate-400">{user.roles.join(', ')}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-slate-400">
                                    {user.roles.includes(UserRole.TEACHER) ? `${user.classSubjectAssignments?.length || 0} classes assigned` : 'N/A'}
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500 dark:text-slate-400">
                                    {user.isBoardMember && <ShieldCheckIcon className="w-6 h-6 text-green-500 inline-block" title="Board Member"/>}
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                                    <button onClick={() => onPasswordReset(user)} className="text-yellow-600 hover:text-yellow-900 dark:text-yellow-400 dark:hover:text-yellow-300" aria-label={`Reset password for ${user.name}`}><KeyIcon className="w-5 h-5"/></button>
                                    <button onClick={() => handleOpenModal(user)} className="text-primary-600 hover:text-primary-900 dark:text-primary-400 dark:hover:text-primary-300" aria-label={`Edit ${user.name}`}><PencilIcon /></button>
                                    <button onClick={() => onTeacherDelete(user.id)} className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300" aria-label={`Delete ${user.name}`}><TrashIcon /></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>

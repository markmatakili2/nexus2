...
import { 
    MOCK_USERS, MOCK_STUDENTS, MOCK_SUBJECTS, MOCK_CLASSES, 
    MOCK_SCHOOL_NAME_DISPLAY, DEFAULT_SCHOOL_DOMAIN, MOCK_TERMS,
...
const loadStateFromStorage = (): AppState => {
    ...
    const storedCurrentTermId = localStorage.getItem('currentTermId') || loadedTerms[0]?.id || '';
...
export function App(): React.ReactElement | null {
  ...
  const handleStudentSave = async (studentData: StudentFormData) => {
    ...
    if (isEditing) {
        ...
        const studentWithSchoolId = { ...finalStudentData, schoolId: appState.currentSchoolId! } as Student;
        updatedStudents = updatedStudents.map(s => s.id === finalStudentData.id ? { ...s, ...studentWithSchoolId } : s);
        ...
        if (originalStudent && originalStudent.parentUsername.toLowerCase() !== newParentUsername.toLowerCase()) {
            const oldParentUser = updatedUsers.find(u => u.username.toLowerCase() === originalStudent.parentUsername.toLowerCase() && u.roles.includes(UserRole.PARENT));
            if (oldParentUser) {
        ...
        const studentWithSchoolId = { ...finalStudentData, id: newStudentId, schoolId: appState.currentSchoolId! } as Student;
        updatedStudents.push(studentWithSchoolId);
    }
...
const handleTeacherSave = async (teacherData: TeacherFormData & {profileImageFile?: File | null; classSubjectAssignments?: { classId: string; subjectIds: string[] }[], signatureImageFile?: File | null}) => {
    ...
    if (isEditing) {
        updatedUsers = updatedUsers.map(u => {
            if (u.id === finalTeacherData.id) {
                return {
                    ...
                    roles: finalTeacherData.roles || u.roles
                };
            }
            return u;
        });
        ...
        const newTeacher: User = {
            ...
            roles: finalTeacherData.roles || [UserRole.TEACHER],
            name: finalTeacherData.name,
  ...
  return (
    ...
    >
      ...
      <Routes>
        ...
        <Route path="/dashboard" element={<ProtectedRoute><DashboardPage {...appState} activeSubjects={activeSubjects} schoolBadgeUrl={appState.schoolBadgeUrl!} /></ProtectedRoute>} />
        ...
        <Route path="/analysis/merit-list" element={<ProtectedRoute roles={[UserRole.ADMIN, UserRole.TEACHER]}><MeritListPage {...appState} activeSubjects={activeSubjects} onComparisonTermChange={handleComparisonTermChange} schoolBadgeUrl={appState.schoolBadgeUrl!} actualSchoolName={appState.actualSchoolName} /></ProtectedRoute>} />
        ...
        <Route path="/analysis/exam-analysis" element={<ProtectedRoute roles={[UserRole.ADMIN, UserRole.TEACHER]}><AdminExamAnalysisPage {...appState} activeSubjects={activeSubjects} schoolBadgeUrl={appState.schoolBadgeUrl!} /></ProtectedRoute>} />
        
        {/* Admin-Only Routes */}
        <Route path="/reports/generate" element={<ProtectedRoute roles={[UserRole.ADMIN]}><GenerateReportsPage {...appState} activeSubjects={activeSubjects} /></ProtectedRoute>} />
        ...
        <Route path="/admin/mark-sheets" element={<ProtectedRoute roles={[UserRole.ADMIN]}><AdminMarkSheetsPage {...appState} activeSubjects={activeSubjects} schoolBadgeUrl={appState.schoolBadgeUrl!} /></ProtectedRoute>} />
        ...
        <Route path="/admin/exam-config" element={<ProtectedRoute roles={[UserRole.ADMIN]}><AdminExamConfigPage initialTerms={appState.terms} initialExamSessions={appState.examSessions} onConfigSave={handleConfigSave} currentSchoolId={appState.currentSchoolId!} /></ProtectedRoute>} />

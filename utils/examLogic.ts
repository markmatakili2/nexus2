...
import { UNIFIED_GRADING_SCALE, SCIENCE_MATH_GRADING_SCALE, SCIENCE_MATH_SUBJECT_IDS } from '../constants';
...
export const calculateSubjectGradeAndPoints = (score: number | null, subjectId?: string): GradePoint & { remarks: string } => {
  ...
  const isScienceOrMath = subjectId && SCIENCE_MATH_SUBJECT_IDS.includes(subjectId);
  const gradingScale = isScienceOrMath ? SCIENCE_MATH_GRADING_SCALE : UNIFIED_GRADING_SCALE;
...
const generateStudentTermSummary = (
  ...
  const subjectTermResults: Partial<SubjectTermResult>[] = subjectsForThisStudent.map(subject => {
      ...
      const termGradeDetails = calculateSubjectGradeAndPoints(finalReportedScore, subject.id);
...
export const generateStudentReport = (
  ...
  const subjectTermResults: SubjectTermResult[] = subjectsForThisStudentReport.map(subject => {
    ...
    termExamSessions.forEach(session => {
        const mark = allMarks.find(m => m.studentId === student.id && m.subjectId === subject.id && m.examSessionId === session.id);
        const score = mark ? mark.score : null;
        const gradeDetails = calculateSubjectGradeAndPoints(score, subject.id);
  ...
  const principal = allUsers.find(u => u.roles.includes(UserRole.ADMIN) && u.name.toLowerCase().includes('principal'));
...
export const getStudentPerformanceHistory = (
    ...
    const academicYear = currentTermForYear.year;
    const termsForAcademicYear = allTerms.filter(term => term.year === academicYear && term.schoolId === student.schoolId);


    termsForAcademicYear.forEach(term => {
  ...
  allSubjects: Subject[]
    ...
    const subjectAnalyses: SubjectClassAnalysis[] = activeSubjects.map(subject => {
        ...
        const gradeDistribution = subjectScores.reduce((dist, s) => {
            const grade = calculateSubjectGradeAndPoints(s.score, subject.id).grade;
            if (dist.hasOwnProperty(grade)) {
                dist[grade]++;
            }
            return dist;
        ...
        if (sortedStudents.length > 0) {
            ...
            for (let i = 0; i < sortedStudents.length; i++) {
                ...
                const { grade } = calculateSubjectGradeAndPoints(sortedStudents[i].score, subject.id);
